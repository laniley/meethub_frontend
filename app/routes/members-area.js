...
export default AuthenticateRoute.extend({
  ...
  prepareController: function(controller, user, response) {
    ...
    this.controllerFor('members-area.index').set('model', user);
    this.controllerFor('members-area.meethubs.index').set('model', user);
    this.controllerFor('members-area.messages.index').set('model', user);
    this.controllerFor('members-area.friends').set('model', user);
    this.controllerFor('members-area.friends.index').set('model', user);

    if(response.friends.data.length > 0)
    {
      for(var i = 0; i < response.friends.data.length; i++)
      {
        this.handleFriendResponse(user, response.friends.data[i].id);
      }
    }
  },

  handleFriendResponse: function(user, friend_fb_id) {
